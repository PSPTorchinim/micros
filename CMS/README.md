...
The CMS includes a comprehensive test suite using Jest for unit and integration testing.
...
- **Health Controller Tests** (6 tests)
  - Status checks and response validation
  - Environment detection
  - Timestamp and uptime reporting
  - Error handling

- **Swagger Controller Tests** (5 tests)
...
When adding new custom controllers or services with business logic, follow this template:

```typescript
import controllerName from '../controllers/controller-name';
...
describe('Controller Name', () => {
  ...
  it('should perform expected behavior', async () => {
    await controllerName.method(ctx);
    
    expect(ctx.status).toBe(200);
    expect(ctx.body).toEqual({ /* expected response */ });
  });
});
```

**Note**: Factory-generated controllers (created using `factories.createCoreController`) are auto-generated by Strapi and don't require custom tests unless they include custom business logic.
