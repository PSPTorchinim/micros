...
export async function seedArticleBlocks(strapi: any) {
  ...
  console.info(
    '[SEED][ARTICLE_BLOCKS] 🔍 Checking for existing article block...',
  );
  ...
  console.info(
    `[SEED][ARTICLE_BLOCKS] ✓ Found ${articleIds.length} published articles`,
  );
  articles.forEach((article: any, index: number) => {
    console.info(
      `[SEED][ARTICLE_BLOCKS]    ${index + 1}. "${article.Title}" (ID: ${article.id})`,
    );
  });

  // PHASE 1: Create or find Article Block without articles relation
  if (!articleBlock) {
    console.info(
      '[SEED][ARTICLE_BLOCKS] ➕ Creating new article block (without articles)...',
    );
    ...
    console.info(
      `[SEED][ARTICLE_BLOCKS] ✓ Found existing article block (ID: ${articleBlock.id})`,
    );
  }

  // PHASE 2: Attach articles relation using entityService with connect syntax
  console.info(
  ...
  try {
    ...
    console.info(
      `[SEED][ARTICLE_BLOCKS] ✓ Connected ${articleIdObjects.length} articles to block`,
    );
  } catch (error: any) {
    console.error(
      '[SEED][ARTICLE_BLOCKS] ❌ Failed to set articles relation:',
      error.message,
    );
  }
  ...
  console.info(
    `[SEED][ARTICLE_BLOCKS] ✅ Updated Article Block (ID: ${articleBlock.id})`,
  );
  console.info(`[SEED][ARTICLE_BLOCKS] 🔍 Verifying articles attachment...`);
  if (!verified) {
    console.warn(
      `[SEED][ARTICLE_BLOCKS] ⚠️  Could not verify updated article block (ID: ${articleBlock.id}) - entity not found after update.`,
    );
    console.info('[SEED][ARTICLE_BLOCKS] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    return;
  }
  ...
  if (attachedCount !== articleIds.length) {
    console.warn(
      `[SEED][ARTICLE_BLOCKS] ⚠️  WARNING: Expected ${articleIds.length} articles but found ${attachedCount}`,
    );
    console.info(
      `[SEED][ARTICLE_BLOCKS] ⚠️  Expected articles: [${articleIds.join(', ')}]`,
    );
  }
  console.info(
    `[SEED][ARTICLE_BLOCKS] ✅ Published: ${articleBlock.publishedAt ? 'Yes' : 'No'}`,
  );
  console.info('[SEED][ARTICLE_BLOCKS] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
}
