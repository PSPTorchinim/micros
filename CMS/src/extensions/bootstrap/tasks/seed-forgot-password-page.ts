...
export async function seedForgotPasswordPage(strapi: any, configId: number) {
  ...
  const count = await strapi.db.query(PAGE_UID).count({ where: { Slug: 'users/forgot-password' } });
  if (count > 0) {
  ...
  if (!existingForgotPage) {
    ...
    strapi.log.info(
      `[SEED][FORGOT_PASSWORD] Created Forgot Password Page (ID: ${forgotPage.id}, Slug: /users/forgot-password)`,
    );
  } else {
